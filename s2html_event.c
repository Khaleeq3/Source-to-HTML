...
static char* preprocessor_res_keyword[] = {"#include", "#define", "#undef", 
								"#ifdef", "#ifndef", "#if", "#elif", "#else", 
								"#endif", "#error", "#warning", "#line", "#pragma", "#", "##", ""
								};
...
pevent_t * pstate_preprocessor_reserve_keyword_handler(FILE *fd, int ch);
pevent_t * pstate_escape_char_handler(FILE *fd, int ch);
...
static int is_preprocessor_keyword(char *word)
{
	int idx = 0;

	while(*preprocessor_res_keyword[idx])
	{
		if(strcmp(preprocessor_res_keyword[idx++], word) == 0)
		...
		switch(state)
		{
			...
			case PSTATE_ASCII_CHAR :
				if((evptr = pstate_ascii_char_handler(fd, ch)) != NULL)
					return evptr;
				break;
			case PSTATE_ESCAPE_CHAR :
				if((evptr = pstate_escape_char_handler(fd, ch)) != NULL)
					return evptr;
				break;
			default : 
...
pevent_t * pstate_idle_handler(FILE *fd, int ch)
{
	...
	switch(ch)
	{
		...
		case '/' :		/* Beginning of Comment */
			...
			break;
		case '#' :	/* Beginning of preproccessor directive */
			pre_ch = ch;
			if(event_data_idx) // we have regular exp in buffer first process that
			{
				fseek(fd, -1L, SEEK_CUR); // unget char
		...
		case 'a' ... 'z' : // could be reserved key word
			...
			{
#ifdef DEBUG
				printf("May be a reserved keyword\n");
#endif
...
pevent_t * pstate_preprocessor_directive_handler(FILE *fd, int ch)
{
	...
	switch(state_sub)
	{
		...
		case PSTATE_SUB_PREPROCESSOR_RESERVE_KEYWORD :
			...
			return pstate_preprocessor_reserve_keyword_handler(fd,ch);
		case PSTATE_SUB_PREPROCESSOR_ASCII_CHAR :
			return pstate_ascii_char_handler(fd, ch);
		case PSTATE_SUB_HEADER_FILE :
...
pevent_t * pstate_sub_preprocessor_main_handler(FILE *fd, int ch)
{
	...
	if(pre_ch == '#')
	{
		...
		switch (ch)
		{
			...
			default:
				pevent_data.data[event_data_idx++] = pre_ch;
				pevent_data.data[event_data_idx++] = ch;
				while((pevent_data.data[event_data_idx++] = getc(fd)) != '\n');
				set_parser_event(PSTATE_IDLE, PEVENT_REGULAR_EXP);
				return &pevent_data;
				break;
	...
	else
	{
		...
		case '<':	/* header file begins here */
		case '\"':
			state_sub = PSTATE_SUB_HEADER_FILE;
			break;
...
pevent_t * pstate_preprocessor_reserve_keyword_handler(FILE *fd, int ch)
{
	...
	case '\"':
		fseek(fd, -1L, SEEK_CUR);
		pevent_data.data[event_data_idx] = '\0';
		if(is_preprocessor_keyword(pevent_data.data) == PREPROCESSOR_KEYWORD)
		{
			set_parser_event(PSTATE_PREPROCESSOR_DIRECTIVE, PEVENT_PREPROCESSOR_DIRECTIVE);
			state_sub = PSTATE_SUB_PREPROCESSOR_MAIN;
			return &pevent_data;
...
pevent_t * pstate_reserve_keyword_handler(FILE *fd, int ch)
{
	...
	case ':':
		...
		word_idx = 0;
		pevent_data.property = is_reserved_keyword(word);
		if(pevent_data.property == RES_KEYWORD_DATA || pevent_data.property == RES_KEYWORD_NON_DATA)
		{
...
pevent_t * pstate_string_handler(FILE *fd, int ch)
{
	...
	switch (ch)
	{
	case '\\':	/* Next character might be '*' */
		fseek(fd, -1L, SEEK_CUR);
		set_parser_event(PSTATE_ESCAPE_CHAR, PEVENT_STRING);
		prev_state = PSTATE_STRING;
		return &pevent_data;
	...
	case '%':
			...
			else
			{
				...
				{	
					char pre_ch = ch;
					pevent_data.data[event_data_idx++] = ch;
					if((ch = getc(fd)) == 'u' && (pre_ch == 'z' || pre_ch == 'l' || pre_ch == 'h'))
					{
						pevent_data.data[event_data_idx++] = ch;
					}
					else if(pre_ch == 'l' && ch == 'f')
					{
						pevent_data.data[event_data_idx++] = ch;
					}
					else
					...
					set_parser_event(PSTATE_STRING, PEVENT_PREPROCESSOR_DIRECTIVE);
					return &pevent_data;
				}
			}
			break;
...
pevent_t * pstate_ascii_char_handler(FILE *fd, int ch)
{
	...
	/*	fseek(fd, -1L, SEEK_CUR);
	 ...
	 case '\'':	/* ascii character end here */
		pevent_data.data[event_data_idx++] = ch;
		set_parser_event(PSTATE_IDLE, PEVENT_ASCII_CHAR);
		return &pevent_data;
		

	 case '\\':	/* Next character might be escape character */
		fseek(fd, -1L, SEEK_CUR);
		set_parser_event(PSTATE_ESCAPE_CHAR, PEVENT_ASCII_CHAR);
		prev_state = PSTATE_ASCII_CHAR;
		return &pevent_data;
...
pevent_t * pstate_escape_char_handler(FILE *fd, int ch)
{
	...
	set_parser_event(prev_state, PEVENT_ESCAPE_CHAR);
	return &pevent_data;
}
